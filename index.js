...
const getAsString = (input, options) => {
	return input.replace(options.urlRegexp, match => linkify(match, options));
};

const getAsDocumentFragment = (input, options) => {
	return input.split(options.urlRegexp).reduce((frag, text, index) => {
...
module.exports = (input, options) => {
        if (options.customUrlRegexp) {
            if (typeof (options.customUrlRegexp) === 'function') {
                options.urlRegexp = options.customUrlRegexp();
            }
            if (typeof (options.customUrlRegexp) === 'string') {
                options.urlRegexp = new RegExp(options.customUrlRegexp, 'g');
            }
        }

	options = Object.assign({
		attributes: {},
                urlRegexp: defaultUrlRegex(),
		type: 'string'
	}, options);
